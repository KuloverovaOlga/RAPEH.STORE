const productCardTabs = () => {
    function showTabContent(tabIndex) {
        ...
        tabContentInfo.forEach((info) => {
          info.style.maxHeight = window.innerWidth > 768 ? '57rem' : '39rem';
        });
        ...
        item.addEventListener('click', (e) => {
            ...
            if (target.classList.contains('product-content__tab-content-button')) {
                ...
                if (item.classList.contains('isActive')) {
                    content.style.maxHeight = window.innerWidth > 768 ? '57rem' : '39rem';
                 setTimeout(() => {
    ...
    window.addEventListener('resize', () => {
            tabContents.forEach(item => {
                if (item.classList.contains('isActive')) {
                    ...
                    item.querySelector('.product-content__tab-info').style.maxHeight = window.innerWidth > 768 ? '57rem' : '39rem'; 
                }
            })
    })
};
